...
class DAV_Element_hrefTest extends PHPUnit_Framework_TestCase {
  ...
  public function testConstructor() {
    ...
    $this->assertSame( array( '/path' ), $objString->URIs, 'DAV_Element_href->URIs be an array with one element when the constructor is called with a string as parameter' );
    $expectedArray = array( '/path1', '/path2' );
    $objArray = new DAV_Element_href( $expectedArray );
    $this->assertSame( $expectedArray  , $objArray->URIs , 'DAV_Element_href->URIs should contain the right values when the constructor is called with an array as parameter' );
  }
